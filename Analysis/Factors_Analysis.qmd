...
```{python}
print(pd.isna(df['bdiM']))
print(df['bdiM'].dtypes)

plt.clf()
bdi_check = sns.regplot(x='bdiM', y='groupn', data=df, logistic=True).set_title("BDI Log Odds Linear Plot")
...
## BDI Logistic Model
```{python}
bdi_model= smf.logit(formula="groupn~bdiM", data=df).fit()
#bdi_model.summary()
```

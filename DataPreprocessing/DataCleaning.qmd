...
## Exporting Datsets
```{python}
fp_df.to_csv("../Data/FactorsData/paranoia_noNaNs", index=False, header=True)
fb_df.to_csv("../Data/FactorsData/bdi_noNaNs", index=False, header=True)
fst_df.to_csv("../Data/FactorsData/anxiety_noNaNs", index=False, header=True)
f_cleandf.to_csv("../Data/FactorsData/factors_noNaNs", index=False, header=True)
```
...
```
This shows something to watch out for when doing analysis. All of the participants of the studies have BPD, so it doesn't make much sense to have no symptoms or BPD in general. Thus, there may be outliers in the data that can be taken out.
...
## Exporting Datset
```{python}
s_temp.to_csv("../Data/SeverityData/severity_noNaNs", index=False, header=True)
```
